...
class HabitatSimAudioEnabledEval(HabitatSim):
    ...
    def get_percentage_distinct_poses(self):
        assert 0.0 <= self._percent_distinct_poses <= 1, "{}, {}. {}"\
            .format(self._percent_distinct_poses, NUM_GRID_POINTS_PER_SCENE[self._current_scene.split("/")[-3]] * 4,
                    self._current_scene.split("/")[-3])
        return self._percent_distinct_poses
    ...
    def get_current_mixed_bin_audio_mag_spec(self):
        ...
        for indx, source_position_index in enumerate(self._source_position_indices):
            ...
            self._gt_mono_audio_components.append(np.stack([mono_audio_this_source_magnitude,
                                                            mono_audio_this_source_phase], axis=-1).astype(np.float16))
